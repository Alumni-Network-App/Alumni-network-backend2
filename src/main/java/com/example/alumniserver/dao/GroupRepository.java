...
public interface GroupRepository extends JpaRepository<Group, Long> {
    ...
    @Query("SELECT DISTINCT g FROM Group g LEFT JOIN g.users u WHERE g.name LIKE %:name% AND (g.isPrivate = FALSE OR size(g.users) > 0 AND u.id = :userId) ORDER BY g.lastUpdated DESC NULLS LAST")
